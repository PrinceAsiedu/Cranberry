...
class HomePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        statistic_box = wx.BoxSizer(wx.HORIZONTAL)
        ...
        self.stf_stat = wx.StaticText(self, -1, str(Controller.Staff(
        ).staff_total()), style=wx.ALIGN_CENTRE_HORIZONTAL | wx.ST_NO_AUTORESIZE)
        ...
        # Fees statistic body
        fee_body = wx.BoxSizer(wx.HORIZONTAL)

        fee_bmp = wx.StaticBitmap(self, -1, wx.Bitmap(IMGS.fee_img))
        self.fee_stat = wx.StaticText(self, -1, str(Controller.Fees(
        ).fee_amount_total()), style=wx.ALIGN_CENTRE_HORIZONTAL | wx.ST_NO_AUTORESIZE)
        self.fee_stat.SetFont(FONT_SMALL)
        self.fee_stat.SetForegroundColour('grey35')
        self.fee_stat.SetBackgroundColour('gainsboro')

        fee_ctrls = [
            (fee_bmp, 1, wx.ALL, 10),
            (self.fee_stat, 1, wx.ALL | wx.ALIGN_CENTER, 10),
        ]

        fee_body.AddMany(fee_ctrls)
        ...
        statistic_box.Add(stu_box, 1, wx.LEFT, 40)
        ...
        sizer.Add(statistic_box, flag=wx.ALL |
                  wx.EXPAND, proportion=1, border=5)
...
class StaffForm(sc.SizedDialog):
    def __init__(self, parent, title):
        ...
        wx.StaticText(pane, -1, "Firstname",
                      wx.DefaultPosition, wx.DefaultSize)
        self.fname = wx.TextCtrl(
            pane, -1, "", size=(150, -1), validator=TextCtrlValidator(ALPHA_ONLY))

        wx.StaticText(pane, -1, "Lastame", wx.DefaultPosition, wx.DefaultSize)
        self.lname = wx.TextCtrl(
            pane, -1, "", size=(150, -1), validator=TextCtrlValidator(ALPHA_ONLY))

        sex_lbl = wx.StaticText(
            pane, -1, "Gender", wx.DefaultPosition, wx.DefaultSize)
        ...
        self.phone = wx.TextCtrl(
            pane, -1, "", size=(150, -1), validator=TextCtrlValidator(DIGIT_ONLY))

        wx.StaticText(pane, -1, 'Email', wx.DefaultPosition, wx.DefaultSize)
        self.email = wx.TextCtrl(
            pane, -1, "", size=(200, -1), validator=TextCtrlValidator(ALPHA_NUM))

        wx.StaticText(pane, -1, "Address", wx.DefaultPosition, wx.DefaultSize)
        self.address = wx.TextCtrl(
            pane, -1, "", size=(150, -1), validator=TextCtrlValidator(PRINTABLE))

        wx.StaticText(pane, -1, 'Category', wx.DefaultPosition, wx.DefaultSize)
        self.category = wx.Choice(
            pane, -1, choices=['Administrator', 'Teacher', 'Other'])

        wx.StaticText(pane, -1, 'Specialty',
                      wx.DefaultPosition, wx.DefaultSize)
        self.specialty = wx.TextCtrl(
            pane, -1, size=(150, -1), validator=TextCtrlValidator(PRINTABLE))

        wx.StaticText(pane, -1, 'Date Of Hiring',
                      wx.DefaultPosition, wx.DefaultSize)
...
class StaffPanel(wx.Panel):
    ...
    def __init__(self, parent, home):
        ...
        for column in columns:
            self.dvlc.AppendTextColumn(
                column[0], width=column[1], mode=dv.DATAVIEW_CELL_EDITABLE)
        ...
        add_staff = pbtn(self, id=wx.ID_ANY,
                         bmp=wx.Bitmap(IMGS.add), label='New')
        ...
        remove_staff = pbtn(self, id=wx.ID_ANY,
                            bmp=wx.Bitmap(IMGS.erase), label='Erase')
        ...
        save_staff = pbtn(self, id=wx.ID_ANY,
                          bmp=wx.Bitmap(IMGS.edit), label='Edit')
    ...
    def GetEmployee(self, dialog):
        ...
        employee_data = [firstname, lastname, gender, phone,
                         email, address, category, specialty, hire_date]
    ...
    def SaveEmployee(self, employee_data):
        ...
        try:
            ...
            save = Controller.Staff(
                fn=fn, ln=ln, sx=sx, nm=ph, em=em, ad=ad, hd=hd, lv=ct, sy=sp)
            ...
            for row in range(rows):
                if self.dvlc.IsRowSelected(row):
                    if wx.MessageBox('Are you sure you want to continue', 'Delete Employee?', style=wx.YES_NO) == wx.YES:
                        ...
                        employee = str(self.dvlc.GetValue(
                            row, 1) + ' ' + self.dvlc.GetValue(row, 2))
                    ...
                    wx.MessageBox('Please select an employee first',
                                  'Error', wx.ICON_ERROR)
...
class StudentForm(sc.SizedDialog):
    def __init__(self, parent, title=None):
        ...
        wx.StaticText(pane, -1, "Date of Birth",
                      wx.DefaultPosition, wx.DefaultSize)
        ...
        wx.StaticText(pane, -1, 'Parent Name',
                      wx.DefaultPosition, wx.DefaultSize)
        self.parent_name = wx.TextCtrl(
            pane, -1, size=(150, -1), validator=TextCtrlValidator(ALPHA_ONLY))

        wx.StaticText(pane, -1, 'Level', wx.DefaultPosition, wx.DefaultSize)
        self.level = wx.Choice(
            pane, -1, choices=['JHS', 'SHS', 'Professional'])

        wx.StaticText(pane, -1, 'Type of Student',
                      wx.DefaultPosition, wx.DefaultSize)
        ...
        wx.StaticText(pane, -1, 'Addmission Date',
                      wx.DefaultPosition, wx.DefaultSize)
...
class StudentPanel(wx.Panel):
    ...
    def __init__(self, parent, home):
        ...
        remove_student = pbtn(self, id=wx.ID_ANY,
                              bmp=wx.Bitmap(IMGS.erase), label='Erase')
        ...
        edit_student = pbtn(self, id=wx.ID_ANY,
                            bmp=wx.Bitmap(IMGS.edit), label='Edit')
    ...
    def OnNewStudent(self, event):
        ...
        # Form dialog as a context manager
        with StudentForm(self, "Student Registration") as dlg:
            dlg.CenterOnScreen()
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def GetStudentFormDetails(self, dialog):
        ...
        student_data = [firstname, lastname, gender, parent_name,
                        dob, phone, email, address, level, stype, adm_date]
        ...
        try:
            ...
            if wx.MessageBox('Are you sure you want to continue', 'Delete Student?', style=wx.YES_NO) == wx.YES:
                for row in range(rows):
                    if self.dvlc.IsRowSelected(row):
                        sid = self.dvlc.GetValue(row, 0)
                        student = str(self.dvlc.GetValue(
                            row, 1) + ' ' + self.dvlc.GetValue(row, 2))
                        Controller.Student().delete_student(sid)
                        self.dvlc.DeleteItem(row)

                        student_total = str(
                            Controller.Student().student_total())
                        self.home.stu_stat.SetLabel(student_total)
                        notify = adv.NotificationMessage(
...
class CourseForm(sc.SizedDialog):
    ...
    def __init__(self, parent, title=None):
        ...
        wx.StaticText(pane, -1, 'Name of Teacher',
                      wx.DefaultPosition, wx.DefaultSize)
        self.teacher = wx.TextCtrl(
            pane, -1, '', size=(200, -1), validator=TextCtrlValidator(ALPHA_ONLY))

        wx.StaticText(pane, -1, "Duration", wx.DefaultPosition, wx.DefaultSize)
        self.duration = wx.TextCtrl(
            pane, -1, "", size=(200, -1), validator=TextCtrlValidator(ALPHA_NUM))

        wx.StaticText(pane, -1, "Level", wx.DefaultPosition, wx.DefaultSize)
        self.level = wx.Choice(
            pane, -1, choices=["JHS", "SHS", "Professional"], size=(200, -1))

        wx.StaticText(pane, -1, 'Price', wx.DefaultPosition, wx.DefaultSize)
        self.price = wx.TextCtrl(
            pane, -1, "", size=(200, -1), validator=TextCtrlValidator(DIGIT_ONLY))

        wx.StaticText(pane, -1, "Status", wx.DefaultPosition, wx.DefaultSize)
        self.status = wx.Choice(pane, -1, size=(200, -1),
...
class CoursePanel(wx.Panel):
    ...
    def __init__(self, parent, home):
        ...
        for cs in self.courses:
            course = [cs.cid, cs.name, cs.teacher,
                      cs.duration, cs.level, cs.price, cs.status]
        ...
        col = wx.Colour('dodgerblue')
        # new course button
        new_course = pbtn(self, id=wx.ID_ANY,
                          bmp=wx.Bitmap(IMGS.add), label='New')
        ...
        remove_course = pbtn(self, id=wx.ID_ANY,
                             bmp=wx.Bitmap(IMGS.erase), label='Erase')
        ...
        save_course = pbtn(self, id=wx.ID_ANY,
                           bmp=wx.Bitmap(IMGS.edit), label='Edit')
    ...
    def SaveCourse(self, course_data):
        ...
        try:
            ...
            save = Controller.Courses(
                name=name, teacher=teacher, duration=duration, level=level, price=price, status=status)
            save.add_course()
    ...
    def append_course(self):
        ...
        course_data = [cs.cid, cs.name, cs.teacher,
                       cs.duration, cs.price, cs.level, cs.status]
                ...
                with CourseForm(self, 'Course Data Modification') as dlg:
                    ...
                    if dlg.ShowModal() == wx.ID_OK:
                        try:
                            ...
                            Controller.Courses().edit_course(
                                cid, subject, teacher, duration, level, price, status)
...
class FeePanel(wx.Panel):
    ...
    def __init__(self, parent, home):
        ...
        new_payment = pbtn(self, id=wx.ID_ANY, bmp=wx.Bitmap(
            IMGS.add), label='Make Payment')
        ...
        erase_payment = pbtn(self, id=wx.ID_ANY, bmp=wx.Bitmap(
            IMGS.erase), label='Erase Payment')
        ...
        edit_payment = pbtn(self, id=wx.ID_ANY, bmp=wx.Bitmap(
            IMGS.edit), label='Edit Payment')
    ...
    def SavePayment(self, payment_data):
        ...
        try:
            ...
            payment = Controller.Fees(
                amount, payer, receiver, arrears, full, date_paid)
                ...
                with PaymentForm(self, 'Fee Payment Modification') as dlg:
                    ...
                    if dlg.ShowModal() == wx.ID_OK:
                        try:
                            ...
                            for fee in fees:
                                fee.date_paid = datetime.strftime(
                                    fee.date_paid, '%x')
                                fee = [
                                    fee.fid, fee.amount, fee.payer, fee.receiver,
                                    fee.arrears, fee.full_payment, fee.date_paid
...
class ItemForm(sc.SizedDialog):
    def __init__(self, parent, title=None):
        ...
        nm_lbl = wx.StaticText(
            pane, -1, "Name", wx.DefaultPosition, wx.DefaultSize)
        self.name = wx.TextCtrl(
            pane, -1, "", size=(300, -1), validator=TextCtrlValidator(ALPHA_ONLY))

        dt_lbl = wx.StaticText(pane, -1, "Description",
                               wx.DefaultPosition, wx.DefaultSize)
        self.description = wx.TextCtrl(
            pane, -1, "", size=(300, -1), validator=TextCtrlValidator(ALPHA_NUM))

        qt_lbl = wx.StaticText(pane, -1, "Quantity",
                               wx.DefaultPosition, wx.DefaultSize)
        self.quantity = qty = wx.TextCtrl(
            pane, -1, "", size=(300, -1), validator=TextCtrlValidator(DIGIT_ONLY))

        st_lbl = wx.StaticText(pane, -1, 'State/Condition',
                               wx.DefaultPosition, wx.DefaultSize)
        self.state = wx.Choice(
            pane, -1, size=(300, -1), choices=['Not Good', 'Normal', 'Good', 'Excellent'])

        ct_lbl = wx.StaticText(
            pane, -1, "Cost", wx.DefaultPosition, wx.DefaultSize)
        self.cost = cs = wx.TextCtrl(
            pane, -1, "", size=(300, -1), validator=TextCtrlValidator(DIGIT_ONLY))

        dc_lbl = wx.StaticText(
            pane, -1, "Discount(In Money)", wx.DefaultPosition, wx.DefaultSize)
        self.discount = wx.TextCtrl(
            pane, -1, '', size=(300, -1), validator=TextCtrlValidator(DIGIT_ONLY))
...
class InventoryPanel(wx.Panel):
    ...
    def __init__(self, parent, home):
        ...
        col = wx.Colour('dodgerblue')
        # new item button
        new_item = pbtn(self, id=wx.ID_ANY,
                        bmp=wx.Bitmap(IMGS.add), label='New')
        ...
        # remove item button
        remove_item = pbtn(self, id=wx.ID_ANY,
                           bmp=wx.Bitmap(IMGS.erase), label='Erase')
        ...
        # save changes made to item info button
        save_item = pbtn(self, id=wx.ID_ANY,
                         bmp=wx.Bitmap(IMGS.edit), label='Edit')
                ...
                with ItemForm(self, 'Item Data Modification') as dlg:
                    ...
                    if dlg.ShowModal() == wx.ID_OK:
                        try:
                            ...
                            item = Controller.Inventory(
                                name, description, quantity, state, cost, total, discount, p_date)
...
class SmsPad(sc.SizedDialog):
    def __init__(self, parent):
        ...
        sc.SizedDialog.__init__(self, None, -1, 'SMS',
                                size=(400, 305), style=FLAGS)
        cPane = self.GetContentsPane()
        pane = sc.SizedScrolledPanel(cPane, wx.ID_ANY)
        pane.SetSizerProps(expand=True, proportion=1)
        pane.SetSizerType("vertical")
        ...
        self.recipient = wx.TextCtrl(
            pane, -1, value="", style=wx.TE_PROCESS_ENTER)
        ...
        self.body = wx.TextCtrl(
            pane, -1, size=(-1, 110), style=wx.TE_MULTILINE)
...
class MailPad(sc.SizedDialog):
    def __init__(self, parent):
        ...
        sc.SizedDialog.__init__(self, parent, -1, 'Mail',
                                size=(550, 450), style=FLAGS)
        cPane = self.GetContentsPane()
        pane = sc.SizedScrolledPanel(cPane, wx.ID_ANY)
        pane.SetSizerProps(expand=True, proportion=1)
        pane.SetSizerType("vertical")
...
class LoginDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, title, size=wx.DefaultSize, pos=wx.DefaultPosition, style=wx.DEFAULT_DIALOG_STYLE,):
        ...
        main_sizer.Add(user_sizer, 0, wx.GROW |
                       wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5)
        main_sizer.Add(p_sizer, 0, wx.GROW |
                       wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5)
        main_sizer.Add(line, 0, wx.GROW |
    ...
    def onLogin(self, event=None):
        ...
        try:
            ...
            wx.MessageBox('Error signing in\n {}'.format(
                AuthenticationError(err_msg)), 'Login Error')
...
class NewUserForm(sc.SizedDialog):
    def __init__(self, parent, id):
        ...
        sc.SizedDialog.__init__(
            self, parent, id, 'New User', size=(270, 270), style=FLAGS)
        ...
        btnsizer = self.CreateButtonSizer(
            wx.OK | wx.CANCEL | wx.ALIGN_CENTRE_HORIZONTAL)
...
class EditUser(sc.SizedDialog):
    def __init__(self, parent, id):
        ...
        sc.SizedDialog.__init__(
            self, parent, id, 'Edit User', size=(300, 250), style=FLAGS)
        self.SetIcon(wx.Icon(APP_ICON))
        ...
        wx.StaticText(pane, -1, 'Username')
        self.uname = wx.TextCtrl(
            pane, -1, '', validator=TextCtrlValidator(ALPHA_ONLY))

        wx.StaticText(pane, -1, 'New username')
        self.nuname = wx.TextCtrl(
            pane, -1, '', validator=TextCtrlValidator(ALPHA_ONLY))
        ...
        self.passw.SetValidator(
            validator=PasswordValidator(PRINTABLE, self.passw1))

        self.SetButtonSizer(self.CreateButtonSizer(
            wx.OK | wx.CANCEL | wx.ALIGN_CENTER_HORIZONTAL))
...
class AppFrame(wx.Frame):
    ...
    def __init__(self, parent, title=None):
        FLAGS = (wx.FULL_REPAINT_ON_RESIZE | wx.MINIMIZE_BOX |
                 wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.MAXIMIZE_BOX)
        super(AppFrame, self).__init__(
            parent=parent, title=title,  style=FLAGS)
        ...
        self.toolbar.AddTool(60, 'Menu', wx.Bitmap(
            IMGS.menu_icon), shortHelp='Menu')
        ...
        self.right_panel = rpnl = wx.Panel(
            pnl, style=wx.TAB_TRAVERSAL | wx.CLIP_CHILDREN)
        ...
        pages = [
            ...
            (InventoryPanel(nb, self.home), "Inventory",
             wx.Bitmap(IMGS.inventory_icon))  # ,
            # (ReportPanel(nb, self.home), "Reports", wx.Bitmap(IMGS.report_icon))
        ]

        for page, label, bm in pages:
            nb.AddPage(page, label, bitmap=bm)
        ...
        self.cal = cal = adv.CalendarCtrl(
            rpnl, -1, date=wx.DefaultDateTime, style=adv.CAL_SHOW_HOLIDAYS)
    ...
    def OnRemoveUser(self, event):
        self.du_frame = wx.Frame(
            None, -1, 'Remove User', size=(400, 100), style=wx.CENTRE | wx.CAPTION | wx.CLOSE_BOX)
        ...
        self.uname = wx.TextCtrl(
            pnl, -1, '', validator=TextCtrlValidator(ALPHA_ONLY), style=wx.TE_PROCESS_ENTER)
        ...
        box.Add(btn, 0, wx.TOP | wx.LEFT | wx.RIGHT |
                wx.ALIGN_CENTRE_VERTICAL, 10)
    ...
    def SendMail(self, dialog):
        try:
            ...
            notify = adv.NotificationMessage(
                title="Email",
                message=f"Email not sent! {str(error)}",
                parent=None, flags=wx.ICON_ERROR)
            notify.Show(timeout=40)
            raise error
...
class CranberryInitScreen(adv.SplashScreen):
    def __init__(self):
        ...
        adv.SplashScreen.__init__(
            self, bmp, adv.SPLASH_CENTRE_ON_SCREEN | adv.SPLASH_TIMEOUT, 5000, None, -1)
