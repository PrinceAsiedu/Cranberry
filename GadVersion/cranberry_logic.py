...
class Student():
    ...
    def fetch_student(self, sid):
        student = {}
        try:
            student = self.session().get_a_student(sid)
        except Exception as error:
            raise error
        finally:
            return student
    ...
    def delete_student(self, sid):
        try:
            result = self.session().remove_a_student(sid)
        except Exception as error:
            raise error
    ...
    def edit_student_data(self, sid, fname="", lname="", sex="", bd="", parent="",
        ...
        try:
            ...
            student.edit_student(sid, fname, lname, sex, bd,
                                 parent, phone, email, addr, lvl, ste, adate)
...
class Staff():
    ...
    def fetch_worker(self, wid):
        try:
            worker = self.session().get_worker(wid)
            return worker
        except Exception as error:
            raise error
    ...
    def all_workers(self):
        workers = []
        try:
            workers = self.session().get_all_workers()
        except Exception as error:
            raise error
        finally:
            return workers
    ...
    def delete_worker(self, wid):
        try:
            result = self.session().remove_worker(wid)
        except Exception as error:
            raise error
...
class Courses():
    ...
    def all_courses(self):
        courses = []
        try:
            courses = self.session().get_all_courses()
        except Exception as error:
            raise error
        finally:
            return courses
    ...
    def edit_course(self, cid, name="", teacher="", duration="", level="", price="", status=""):
        try:
            course = self.session()
            course.modify_course_info(
                cid, name, teacher, duration, level, price, status)
    ...
    def delete_course(self, cid):
        try:
            self.session().remove_course(cid)
        except Exception as error:
            raise error
...
class Fees():
    ...
    def all_fees(self):
        fees = []
        try:
            fees = self.session().get_all_fees()
        except Exception as error:
            raise error
        finally:
            return fees
...
class Admin(model.Access_Session):
    ...
    def add_user(self, username, password):
        ...
        try:
            self.create_user(name=username, passw=password)
        except Exception as error:
            raise error
    ...
    def edit_user(self, username, new_uname, password):
        password = hash_password(password)
        try:
            self.update_user(username, new_uname, password)
        except Exception as error:
            raise error
    ...
    def send(self):
        ...
        if _check_for_connection is True:
            try:
                ...
                time = datetime.now()
                self.sent = True
                save_msg(self.__recipient, self.msg,
                         self.sent, time, self.type)
        ...
        for m in msgs:
            if (m.status is False) and (m.mtype == 'sms'):
                try:
                    ...
                    client = sms_client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
                    message = client.messages.create(
                        body=msg, from_=TWILIO_NUMBER, to=rec)
                    time = datetime.now()
